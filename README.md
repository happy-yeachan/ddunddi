...
`storage.objects`는 Supabase가 RLS를 강제해 테이블처럼 끌 수 없다. 버킷 정책을
명시하지 않으면 anon 키 업로드가 전부 거부되므로 `schema.sql`에 select/insert 정책이 함께 있다.

### 보안에 대해

**RLS를 껐고 anon 키로 직접 접근한다. 프로젝트 URL과 anon 키를 아는 사람은
...
**구조**
- RLS를 켜고 제대로 된 인증을 붙이기 (위 보안 항목 참고)
