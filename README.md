...
- `pokes` — 찌르기 기록
- `push_config`, `push_subscriptions` — 암호화된 푸시 키와 기기 구독
- `app_settings` — 앱 설정
...
찌르기 페이지에서 각자 **이 기기 알림 받기**를 켠다. 별도 비밀번호 재확인은 없다.
기존 앱 진입 화면은 유지하며, 푸시 API는 별도 세션 인증 없이 동일 출처 요청과
사용자·구독 값을 검증한다. 동일 출처 검사는 사용자 인증을 대신하지 않는다.
아이폰·아이패드는 iOS 16.4 이상에서 홈 화면에 추가한 앱으로 열어 알림을 허용해야 한다.
알림을 누르면 찌르기 페이지가 열린다. 전송 성공은 푸시 서비스 접수 기준이며,
집중 모드·기기 네트워크·OS 설정에 따라 실제 표시가 지연되거나 제한될 수 있다.

개발/초기 배포: `npm run db:push` 후 `npm run push:setup`을 한 번 실행한다.
VAPID 키는 무작위로 생성해 DB에 암호화 저장하며 재실행해도 유지된다.
암호화에는 기존 서버 전용 `GATE_PASSWORD`를 사용하므로 로컬과 Vercel의 값이 같아야 한다.
비밀번호를 바꾸기 전에는 기존 키와 구독을 새 비밀번호로 재암호화해야 한다.
새 환경변수는 없다. 구독 주소와 개인키는 클라이언트에 반환하거나 로그에 출력하지 않는다.
기존 anon 접근 구조는 유지하되 DB에는 암호문만 저장하고 발송 시 복호화·대상 검증한다.
외부 URL 전송은 알려진 푸시 서비스로 제한한다. 10초 간격 제한과 요청 ID 중복 방지를 적용한다.
서비스 워커는 알림 전용이며 페이지를 캐시하지 않는다.

캘린더에서 새 일정·사진·일기를 저장하면 같은 기기 구독으로 상대에게 알림을 보낸다.
한 번의 저장을 하나의 알림으로 묶으며, 수정·삭제만 한 경우에는 발송하지 않는다.
부분 저장 실패 시에도 실제 저장된 항목만 서버에서 확인한다. 기록별 발송 시도는
`calendar_notification_claims`에 남겨 중복 발송을 막는다. 전송 실패는 저장 결과와
분리해 안내하며 자동 재발송하지 않는다. 알림을 누르면 해당 캘린더 날짜가 열린다.
