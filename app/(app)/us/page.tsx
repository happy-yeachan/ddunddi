...
export default function UsPage() {
  ...
  return <main className="mx-auto max-w-md px-5 pb-8 pt-[calc(1.25rem+env(safe-area-inset-top))]">
    ...
    {loading ? <p className="py-20 text-center text-[#af798b]">우리의 오늘을 불러오는 중…</p> : <>
      ...
      {!error && (!profiles[1]?.name.trim() || !parseDay(profiles[1]?.birth_date)) && <Link href="/us/settings" className="mt-4 flex items-center justify-between rounded-2xl border border-[#efdce3] bg-white p-4 text-sm text-[#9e4e6b]"><span>우리 소개 채우기</span><span>→</span></Link>}
