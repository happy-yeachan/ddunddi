...
export default function UsPage() {
  ...
  return <main className="mx-auto max-w-md px-6 pt-[calc(2rem+env(safe-area-inset-top))] pb-8">
    <Link href="/us/settings" className="mb-5 inline-block text-sm text-app-text">← 설정으로</Link>
    <h1 className="mb-2 text-2xl font-bold">소개서 편집</h1>
    <p className="mb-6 text-sm text-app-muted">두 사람의 소개를 관리해요</p>
    <Link href="/calendar/settings" className="mb-5 inline-block text-sm text-app-text">사귄 날짜 · 기념일은 캘린더 설정에서 →</Link>
    ...
    {loading ? <p className="py-10">불러오는 중…</p> : <form onSubmit={submit} className="space-y-4">
      ...
      <div>
        <p className="mb-2 text-sm font-semibold">사진 <span className="font-normal text-app-muted">(선택)</span></p>
        <div className="flex items-center gap-4 rounded-2xl bg-white p-4">
          ...
          <div className="min-w-0">
            <label className="relative inline-flex cursor-pointer rounded-full bg-app-soft px-4 py-2 text-sm font-semibold text-app-text focus-within:ring-2 focus-within:ring-app-accent">
            ...
            <p id="profile-photo-status" className="mt-2 break-all text-xs text-app-muted">{uploading ? "사진을 올리고 있어요" : photoName || (form.photo_path ? "저장된 사진을 사용 중이에요" : "기본 이미지를 사용해요")}</p>
            {form.photo_path && <button type="button" onClick={() => { change("photo_path", null); setPhotoName(""); setMessage("기본 이미지로 바꿨어요. 저장을 눌러 완료해주세요"); }} className="mt-2 text-xs text-app-text underline">기본 이미지로 변경</button>}
          </div>
        </div>
        <p className="mt-2 text-xs text-app-muted">사진을 올리지 않아도 기본 이미지로 저장할 수 있어요.</p>
      </div>
      <Field label="이름" value={form.name} onChange={(v) => change("name", v)} placeholder="이름을 적어주세요" />
      <label className="block"><span className="mb-2 block text-sm font-semibold">생년월일</span><input type="text" inputMode="numeric" pattern="\d{4}-\d{2}-\d{2}" placeholder="YYYY-MM-DD" value={form.birth_date} onChange={(e) => change("birth_date", formatDateInput(e.target.value))} className="w-full rounded-2xl border border-app-border bg-white px-4 py-3 outline-none placeholder:text-[#d8b6c0] focus:border-app-accent" /></label>
      ...
      <button disabled={saving || uploading || loadFailed} className="w-full rounded-2xl bg-app-accent py-4 text-lg font-semibold text-app-on-accent disabled:opacity-50">{uploading ? "사진 업로드 중…" : saving ? "저장 중…" : "소개서 저장"}</button>
      </fieldset>
...
function Field({ label, value, onChange, placeholder, area }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; area?: boolean }) {
  return <label className="block"><span className="mb-2 block text-sm font-semibold">{label}</span>{area ? <textarea value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} rows={3} className="w-full resize-none rounded-2xl border border-app-border bg-white px-4 py-3 outline-none placeholder:text-[#d8b6c0] focus:border-app-accent" /> : <input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full rounded-2xl border border-app-border bg-white px-4 py-3 outline-none placeholder:text-[#d8b6c0] focus:border-app-accent" />}</label>;
}
