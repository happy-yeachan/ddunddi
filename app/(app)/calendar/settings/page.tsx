...
export default function CalendarSettings() {
  ...
  return <main className="mx-auto max-w-md px-6 py-8 pt-[calc(2rem+env(safe-area-inset-top))]">
    <Link href="/calendar" className="text-sm text-app-text">← 캘린더로</Link>
    <h1 className="mb-6 mt-5 text-2xl font-bold">캘린더 설정</h1>
    <form onSubmit={save} className="space-y-5 rounded-app bg-white p-5">
      <fieldset disabled={loading || saving || !loaded} className="space-y-5 disabled:opacity-50">
        <label className="block text-sm">사귄 날짜<input inputMode="numeric" placeholder="YYYY-MM-DD" value={date} onChange={(e) => { const d=e.target.value.replace(/\D/g, "").slice(0,8); setDate([d.slice(0,4),d.slice(4,6),d.slice(6,8)].filter(Boolean).join("-")); }} className="mt-2 w-full rounded-xl border border-app-border p-3" /></label>
        <p className="text-xs text-app-muted">사귄 날짜가 없어도 생일 표시를 설정할 수 있어요.</p>
        ...
        <p className="text-xs leading-5 text-app-muted">두 사람에게 함께 적용돼요. 대한민국 공휴일은 항상 표시해요.</p>
        <button className="w-full rounded-2xl bg-app-accent py-3 font-semibold text-app-on-accent">{loading ? "불러오는 중…" : saving ? "저장 중…" : "설정 저장"}</button>
      </fieldset>
